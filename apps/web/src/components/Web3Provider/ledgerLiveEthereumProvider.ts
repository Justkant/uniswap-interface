...
export class LedgerLiveEthereumProvider extends EventEmitter implements EIP1193Provider {
  ...
  private handleEventSourceMessage = (event: MessageEvent) => {
    ...
    // TODO: check for our source instead and only handle them
    if (
      data.source?.startsWith('react-devtools') ||
      data.source?.startsWith('@devtools') ||
      data.source?.startsWith('webpack') ||
      data.type?.startsWith('webpack') ||
      data.target?.startsWith('metamask')
    ) {
      return
    }
      ...
      if ('type' in data) {
        switch (data.type) {
          ...
          case 'connect:response':
            ...
            this.emitConnect(data.data.chainId)
            break
  ...
  private emitConnect(chainId: string) {
