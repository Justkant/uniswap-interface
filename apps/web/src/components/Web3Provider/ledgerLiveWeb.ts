import { ChainNotConfiguredError, createConnector, type Connector } from '@wagmi/core'
import {
...
import { LedgerLiveWebSDK } from './ledgerLiveWebSDK'

type Properties = {
  onConnect(connectInfo: ProviderConnectInfo): void
  onDisplayUri(uri: string): void
}
type Listener = Parameters<LedgerLiveEthereumProvider['on']>[1]

ledgerLiveWeb.type = 'ledgerLiveWeb' as const
export function ledgerLiveWeb() {
  let sdk: LedgerLiveWebSDK
  let provider: LedgerLiveEthereumProvider | undefined
  let providerPromise: Promise<LedgerLiveEthereumProvider>
  ...
  return createConnector<LedgerLiveEthereumProvider, Properties>((config) => ({
    id: 'ledgerLiveWeb',
    name: 'Ledger Live',
    type: ledgerLiveWeb.type,
    async setup() {
        ...
        if (
          ...
          try {
            ...
            if (addEthereumChainParameter?.blockExplorerUrls)
              ...
              blockExplorerUrls = [blockExplorer.url, ...Object.values(blockExplorers).map((x) => x.url)]
